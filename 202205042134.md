...
Variables could be defined in #[playbooks](202204272036.md) and
#[inventory](202204272043.md). However, it is recommended to put variables,
primarily credentials that are essential for connection such as username and
password, into inventory.

**Note**: Variables stored in #[modules](202204300820.md) are called *facts*.
...
For conditionals, we could use the #[module's](202204300820.md) `when`. One
can use comparison operators such as `==`, `!=`, `>`, `>=`, `<` and
`<=`, or `in` keyword to check whether the value is found in a list. It has
some limitation for potential large amount of tasks. For this, it is advised to
use handler or separate the tasks into a #[playbook](202204272036.md) and then
included later. The usage is shown as below:
...
variables. That being said, using `loop` is unfavourable due to the
performance hit. A lot of times, #[module](202204300820.md) has better
built-in than `loop`. The following code shows how to use `loop`:
