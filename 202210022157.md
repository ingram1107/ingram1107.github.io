...
**Note**: Availability should always be measured from the user perspective.
...
Availability must be measured to be **determined**, ideally with *comprehensive
monitoring tools*, also known as [*instrumentation*](202607261701.md), that
are themselves highly available. If there is no instrumentation, system that
supports high volume transaction processing throughout the day and night, such
as credit card processing system or telephone switches, often inherently better
monitored.

An alternative metrics is by mean time between failures (MTBF).

A [High Availability (HA)](202607261046.md) service will *prevent financial
loss* and *productivity loss*, *reduces reactive support costs* (the cost to fix
when things break), and *improves customer satisfaction and loyalty*.
...
- software availability such as [Spanning Tree Protocol
  (STP)](202207081637.md) and Hot Standby Router Protocol (HSRP)
...
- adaptive [fault-tolerance](202607261841.md) (monitors the system's
  state and adapt the system to the errors)
