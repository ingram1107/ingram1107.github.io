...
into the class itself. This satisfies the [SOLID](202202041514.md) principle
as the object class is intact from future changes from addition of operations.
This design pattern involves two class hierarchies: the **element** hierarchy
which implements a `accept()` operation that takes a visitor as an argument,
and the **visitor** hierarchy which implement the operations (each operation is
their own class) on the **element** depending on their type.
